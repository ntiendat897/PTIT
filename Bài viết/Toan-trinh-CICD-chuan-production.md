...
**Workflow:** lập trình viên làm việc trên nhánh (vd `master`); khi muốn phát hành, họ **đánh một tag** theo quy ước môi trường (ví dụ tiền tố `stage-` cho môi trường thử nghiệm, `prod-` cho production). Việc tạo tag chính là "nút bấm phát hành" — nó kích hoạt webhook gọi sang hệ thống CI. Cách này giúp mọi lần phát hành đều có **dấu mốc rõ ràng, truy vết được**.
